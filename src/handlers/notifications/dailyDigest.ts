 ...
 *
 * Runs daily at 9:00 AM (configurable per-user timezone) to compile and send
 * outstanding notifications as a digest email to users with DAILY preference.
 */
...
export const handler: ScheduledHandler = async (event, context) => {
  ...
  logLambdaInvocation('dailyDigest', event, context.awsRequestId);
  ...
  try {
    ...
    logJobEvent(context.awsRequestId, 'complete', 'DAILY', {
      ...metrics,
      durationMs: metrics.completedAt.getTime() - startedAt.getTime(),
    });

    logLambdaCompletion('dailyDigest', Date.now() - startedAt.getTime(), context.awsRequestId);

    return;
  } catch (err) {
    logger.error('Daily digest job failed', err as Error);
    metrics.errorCount++;
    metrics.completedAt = new Date();
    await publishJobMetrics(metrics);
    logJobEvent(context.awsRequestId, 'error', 'DAILY', { runId, error: (err as Error).message });
    throw err;
  }
};
