/**
 * Integration Tests: Multi-invite selection
 */
...
describe('Multi-invite selection', () => {
  beforeEach(() => {
    process.env['AWS_SAM_LOCAL'] = 'true';
    jest.clearAllMocks();
  });

  it('accepts only the selected invite', async () => {
    ...
    expect(command.input.TransactItems[0].Update.Key.SK).toBe('INVITATION#invite-1');
  });
});
